...
namespace FactoryPlanner.Core.Stores
{
    public class ObjectCache<T, V> : IObjectCache<T, V>
    {
        ...
        public OperationResult TryGetRange(T[] keys, out V[] values) {
            List<V> foundValues = new List<V>();
            List<T> missingKeys = new List<T>();

            foreach (T key in keys) {
                if (cache.TryGetValue(key, out V? value) && value != null) {
            ...
            if (missingKeys.Count > 0) {
                string errorMessage = $"Keys not found in cache or has null value: {string.Join(", ", missingKeys)}";
                logger.LogWarning(errorMessage);
                return new OperationResult(false, errorMessage, false);
            }
